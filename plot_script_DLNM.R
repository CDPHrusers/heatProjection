...
anzip<-read.csv("data/processed/attributable_number_climate_16_11_7.csv")
colnames(anzip)[1]<-"zip"
anzip<-anzip[match(names(dlist), anzip$zip),]
...
# ATTRIBUTABLE FRACTION
afzip$cold<-round(afzip$cold, 0)
afzip$heat<-round(afzip$heat, 0)
...
anzip %>% group_by(climate_zone) %>% summarise( Median.cold = median(cold), Median.heat = median(heat))
